...
import dev.khayin.app.core.network.safeApiCall
...
import dev.khayin.app.core.plugin.resolvePluginSeasonEpisode
...
import dev.khayin.app.data.mapper.toDomain
...
import dev.khayin.app.domain.model.ProxyHeaders
import dev.khayin.app.domain.model.ScraperInfo
import dev.khayin.app.domain.model.Stream
import dev.khayin.app.domain.model.StreamBehaviorHints
...
class StreamRepositoryImpl @Inject constructor(
    ...
    private fun fetchStreamsFromAllSources(
        ...
        try {
                    ...
                    launch {
                        try {
                            ...
                            attemptedFailures += StreamAttemptFailure(
                                ...
                                detail = e.message ?: context.getString(dev.khayin.app.R.string.stream_error_detail_addon_request_failed)
                            )
                        } finally {
            ...
            emit(NetworkResult.Error(e.message ?: context.getString(dev.khayin.app.R.string.stream_error_fetch_failed)))
        }
    }
    ...
    private fun LocalScraperResult.toPluginStream(scraper: ScraperInfo, addonName: String): Stream {
        ...
        val qualityLabel = quality ?: context.getString(dev.khayin.app.R.string.stream_quality_unknown)
     ...
     */
    private fun buildDescription(result: dev.khayin.app.domain.model.LocalScraperResult): String? {
    ...
    override suspend fun getStreamsFromAddon(
        ...
        val addonName = when (addonResult) {
            ...
            else -> context.getString(dev.khayin.app.R.string.stream_addon_unknown)
        }
    ...
    private fun buildMissingStreamFailure(addon: Addon): StreamAttemptFailure {
        return StreamAttemptFailure(
            ...
            detail = context.getString(dev.khayin.app.R.string.stream_error_detail_no_streams_for_id)
        )
    }
    ...
    private fun buildAddonFailure(addon: Addon, error: NetworkResult.Error): StreamAttemptFailure {
        ...
        val normalizedReason = when {
            error.message.contains("Unable to resolve host", ignoreCase = true) ->
                context.getString(dev.khayin.app.R.string.stream_error_detail_addon_unreachable)
            error.message.contains("Failed to connect", ignoreCase = true) ->
                context.getString(dev.khayin.app.R.string.stream_error_detail_addon_connection_failed)
            error.message.contains("timeout", ignoreCase = true) ->
                context.getString(dev.khayin.app.R.string.stream_error_detail_addon_timeout)
            error.message.contains("CLEARTEXT communication", ignoreCase = true) ->
                context.getString(dev.khayin.app.R.string.stream_error_detail_addon_cleartext_blocked)
            error.message.isBlank() ->
                context.getString(dev.khayin.app.R.string.stream_error_detail_addon_request_failed)
