...
import dev.khayin.app.core.tracking.TRACKING_SCROBBLE_DIAGNOSTIC_TAG
...
import dev.khayin.app.core.tracking.scrobbleDiagnosticIdentity
import dev.khayin.app.data.local.SubtitleStyleSettings
import dev.khayin.app.data.repository.PlaybackIssueErrorInput
import dev.khayin.app.data.repository.PlaybackIssuePlaybackSettingsInput
import dev.khayin.app.data.repository.PlaybackIssueReportInput
 ...
 * guard in [dev.khayin.app.core.player.ExternalPlaybackTracker].
 */
        ...
        PlayerEvent.OnRetry -> {
            ...
            if (isTorrentStream && currentInfoHash != null) {
                ...
                launchTorrentSourceStream(
                    stream = dev.khayin.app.domain.model.Stream(
...
internal fun PlayerRuntimeController.buildStreamInfoData(): StreamInfoData {
    ...
    return StreamInfoData(
        ...
        playerEngine = when (currentInternalPlayerEngine) {
            dev.khayin.app.data.local.InternalPlayerEngine.EXOPLAYER -> context.getString(R.string.playback_engine_exoplayer)
            dev.khayin.app.data.local.InternalPlayerEngine.MVP_PLAYER -> context.getString(R.string.playback_engine_mvplayer)
            dev.khayin.app.data.local.InternalPlayerEngine.AUTO -> null
        }
    )
}
