...
internal fun PlayerRuntimeController.attachMpvView(view: NuvioMpvSurfaceView?) {
    ...
    runCatching {
        ...
        val detailedError = it.message ?: context.getString(dev.khayin.app.R.string.player_error_mpv_surface_failed)
        if (
...
internal fun PlayerRuntimeController.initializeMpvPlayer(
    ...
    if (view == null) {
        setLoadingStatus(
            ...
            message = context.getString(dev.khayin.app.R.string.player_loading_building),
            showOverlay = true
        )
        _uiState.update {
            it.copy(
    ...
    runCatching {
        setLoadingStatus(
            phase = "mpv_starting",
            message = context.getString(dev.khayin.app.R.string.player_loading_starting),
            showOverlay = true
        )
        performPendingMpvHardRestartIfNeeded(view)
        view.applyHardwareDecodeMode(mpvHardwareDecodeModeSetting)
        ...
        val detailedError = error.message ?: context.getString(dev.khayin.app.R.string.player_error_mpv_playback_failed)
        if (
            maybeAutoSwitchInternalPlayerOnStartupError(
                detailedError = detailedError,
