...
internal fun PlaybackException.toDisplayMessage(context: android.content.Context): String {
    ...
    if (responseException != null) {
        ...
        val providerHint = when (code) {
            400 -> context.getString(dev.khayin.app.R.string.player_error_stream_blocked)
            401 -> context.getString(dev.khayin.app.R.string.player_error_stream_expired)
            403 -> context.getString(dev.khayin.app.R.string.player_error_stream_blocked)
            404 -> context.getString(dev.khayin.app.R.string.player_error_stream_removed)
            410 -> context.getString(dev.khayin.app.R.string.player_error_stream_expired)
            429 -> context.getString(dev.khayin.app.R.string.player_error_stream_rate_limited)
            500, 502, 503, 504 -> context.getString(dev.khayin.app.R.string.player_error_stream_unavailable)
            else -> ""
        }
        return buildString {
            append("HTTP $code")
    ...
    if (isUnrecognizedFormat) {
        return context.getString(dev.khayin.app.R.string.player_error_source_invalid_content, errorCodeName)
    }
    ...
    if (isRendererError) {
        ...
        val decoderHeader = meaningfulMessage ?: context.getString(dev.khayin.app.R.string.player_error_decoder)
        val unsupported = context.getString(dev.khayin.app.R.string.player_error_unsupported_format, errorCodeName)
...
internal fun Throwable.toDisplayMessage(context: android.content.Context, fallback: String? = null): String {
    ...
    return meaningfulMessage
        ...
        ?: fallback
        ?: context.getString(dev.khayin.app.R.string.player_error_playback_fallback)
}
