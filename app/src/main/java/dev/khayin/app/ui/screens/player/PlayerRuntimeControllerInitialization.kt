...
import dev.khayin.app.core.player.DolbyVisionCodecFallback
import dev.khayin.app.core.player.DolbyVisionBaseLayerPolicy
import dev.khayin.app.core.player.BitrateAwareLoadControl
import dev.khayin.app.core.player.DolbyVisionConversionConfig
import dev.khayin.app.core.player.DolbyVisionConversionStats
import dev.khayin.app.core.player.DolbyVisionExtractorsFactory
import dev.khayin.app.core.player.DoviBridge
import dev.khayin.app.core.player.LastPlaybackDiagnostics
import dev.khayin.app.core.tracking.TrackingScrobbleAction
import dev.khayin.app.ui.screens.settings.MemoryBudget
import dev.khayin.app.data.local.AudioLanguageOption
import dev.khayin.app.data.local.Dv7HandlingMode
import dev.khayin.app.data.local.FrameRateMatchingMode
import dev.khayin.app.data.local.SUBTITLE_LANGUAGE_FORCED
...
internal fun PlayerRuntimeController.initializePlayer(
    ...
    scope.launch {
        try {
            ...
            dev.khayin.app.core.player.dvmkv.DolbyVisionCompatibility.setHdr10BaseLayerModeActive(isHdr10BaseLayerModeActive)
            ...
            val effectiveAudioOutputChannels = if (bluetoothStereoDownmix) {
                dev.khayin.app.data.local.AudioOutputChannels.CHANNELS_2_0
            } else {
            ...
            if (
                maybeAutoSwitchInternalPlayerOnStartupError(
                    detailedError = e.message ?: context.getString(dev.khayin.app.R.string.player_error_initialize_failed),
                    allowEngineFailover = allowEngineFailover
                )
            ) {
                return@launch
            }
            val displayError = e.toDisplayMessage(context, context.getString(dev.khayin.app.R.string.player_error_initialize_failed))
...
private class SubtitleOffsetRenderersFactory(
    ...
    private val audioOutputChannels: dev.khayin.app.data.local.AudioOutputChannels,
...
private fun FfmpegAudioRenderer.applyDownmixSettings(
    ...
    audioOutputChannels: dev.khayin.app.data.local.AudioOutputChannels,
...
private fun PlayerRuntimeController.recordFirstFrameDiagnostics(
    ...
    playerSettings: dev.khayin.app.data.local.PlayerSettings
