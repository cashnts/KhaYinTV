...
import dev.khayin.app.core.debrid.DebridDeviceAuthorization
import dev.khayin.app.core.debrid.DebridDeviceAuthorizationTokenResult
import dev.khayin.app.core.debrid.DebridProviderCapability
import dev.khayin.app.core.debrid.DebridProviders
import dev.khayin.app.core.debrid.supports
import dev.khayin.app.core.qr.QrCodeGenerator
import dev.khayin.app.core.server.DebridFormatterConfigServer
import dev.khayin.app.core.server.DebridFormatterSettings
...
import dev.khayin.app.data.remote.dto.TorboxDeviceTokenDto
import dev.khayin.app.data.remote.dto.TorboxDeviceTokenRequestDto
import dev.khayin.app.data.remote.dto.TorboxEnvelopeDto
import dev.khayin.app.data.remote.api.PremiumizeApi
import dev.khayin.app.data.remote.api.TorboxApi
import dev.khayin.app.domain.model.DEBRID_PREPARE_INSTANT_PLAYBACK_DEFAULT_LIMIT
...
class DebridSettingsViewModel @Inject constructor(
    ...
    suspend fun startDeviceAuthorization(providerId: String): DebridDeviceAuthorization? {
        return when (DebridProviders.byId(providerId)?.id) {
            DebridProviders.TORBOX_ID -> {
                val response = torboxApi.startDeviceAuthorization("KhaYin")
...
data class DebridSettingsUiState(
    ...
    val resolverProviders: List<dev.khayin.app.core.debrid.DebridProvider>
    ...
    val activeResolverProvider: dev.khayin.app.core.debrid.DebridProvider?
    ...
    val cloudLibraryProviders: List<dev.khayin.app.core.debrid.DebridProvider>
        get() = DebridProviders.visible()
            .filter { provider ->
                apiKeyFor(provider.id).isNotBlank() &&
