...
import dev.khayin.app.data.local.StartupSyncPreferences
...
class StartupSyncService @Inject constructor(
    ...
    private val cwEnrichmentCache: dev.khayin.app.data.local.ContinueWatchingEnrichmentCache,
    private val licenseWatchProgressSyncService: LicenseWatchProgressSyncService
) {
    ...
    init {
        scope.launch {
            dev.khayin.app.features.license.LicenseRepository.state.collect { licState ->
                if (licState is dev.khayin.app.features.license.LicenseState.Active) {
                    val profileId = profileManager.activeProfileId.value
                    licenseWatchProgressSyncService.pullRemote(profileId)
                }
            }
        }
        scope.launch {
        ...
        periodicSurfacePullJob = scope.launch {
            while (true) {
                ...
                if (licenseWatchProgressSyncService.getActiveLicenseKey() != null) {
                    licenseWatchProgressSyncService.pullRemote(profileManager.activeProfileId.value)
                }
