...
object FrameRateUtils {
    ...
    private val probeHttpClient by lazy {
        dev.khayin.app.ui.screens.player.PlayerPlaybackNetworking.playbackHttpClient.newBuilder()
    ...
    internal fun warmHttpOriginForAfrProbe(
        ...
        if (headers.none { it.key.equals("User-Agent", ignoreCase = true) }) {
            requestBuilder.header(
                "User-Agent",
                dev.khayin.app.ui.screens.player.PlayerMediaSourceFactory.DEFAULT_USER_AGENT
            )
        }
        headers.forEach { (k, v) ->
    ...
    internal fun fetchHttpRangeToFile(
        ...
        if (headers.none { it.key.equals("User-Agent", ignoreCase = true) }) {
            requestBuilder.header("User-Agent", dev.khayin.app.ui.screens.player.PlayerMediaSourceFactory.DEFAULT_USER_AGENT)
        }
